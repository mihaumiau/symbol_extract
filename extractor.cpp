...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "usage: ./extract.exe <module_path> <symbol_location> [<symbol_name>...] <disassemble_out>" << std::endl;
    ...
    if (FAILED(symbols->SetSymbolPath(argv[2]))) {
    ...
    if (FAILED(result = client->CreateProcess(0, const_cast<char*>((std::string("harness.exe ") + argv[1]).c_str()), DEBUG_PROCESS))){
    ...
    if (FAILED(result = symbols->Reload(argv[1]))) {
